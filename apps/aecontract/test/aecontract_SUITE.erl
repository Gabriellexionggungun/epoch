...
preclaim(PubKey, Name, Options, S) ->
    ...
    {ok, Tx} = aens_preclaim_tx:new(#{ account => PubKey,
                                       nonce => Nonce,
                                       commitment => CHash,
                                       fee => Fee,
                                       ttl => TTL }),
...
claim(PubKey, Name, Salt, Options, S) ->
    ...
    {ok, Tx} = aens_claim_tx:new(#{ account   => PubKey,
                                    nonce     => Nonce,
                                    name      => Name,
                                    name_salt => Salt,
                                    fee       => Fee,
...
aens_update(PubKey, NameHash, Pointers, Options, S) ->
    ...
    {ok, Tx}  = aens_update_tx:new(#{ account    => PubKey,
                                      nonce      => Nonce,
                                      name_hash  => NameHash,
                                      name_ttl   => NameTTL,
                                      pointers   => Pointers,
...
sophia_aens(_Cfg) ->
    ...
    {some, NameKey} = ?call(call_contract, Acc, Ct, lookup_word,   {option, word},   {Name, <<"account_pubkey">>}),
    none            = ?call(call_contract, Acc, Ct, lookup_word,   {option, word},   {Name, <<"oracle_pubkey">>}),
    {some, URL}     = ?call(call_contract, Acc, Ct, lookup_string, {option, string}, {Name, <<"name">>}),
    ok.
